package docconv // import "github.com/luisfurquim/docconv"
...
// Convert a file to array of paragraphs.
func ConvertParagraphs(r io.Reader, mimeType string, readability bool) (*Paragraphs, error) {
   start := time.Now()
...
*/
   case "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
      par, err = ConvertDocxParagraphs(r)
/*
   case "application/vnd.openxmlformats-officedocument.presentationml.presentation":
      body, meta, err = ConvertPptx(r)
...
*/
   ...
   par.MSecs = uint32(time.Since(start) / time.Millisecond)

   return par, nil
}

// ConvertPath converts a local path to text.
func ConvertPathParagraphs(path string) (*Paragraphs, error) {
   mimeType := MimeTypeByExtension(path)
   ...
   return ConvertParagraphs(f, mimeType, true)
}

// ConvertPathReadability converts a local path to text, with the given readability
// option.
func ConvertPathReadabilityParagraphs(path string, readability bool) ([]byte, error) {
   mimeType := MimeTypeByExtension(path)
   ...
   data, err := ConvertParagraphs(f, mimeType, readability)
   if err != nil {
      return nil, err
   }
   return json.Marshal(data)
